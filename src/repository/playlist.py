...
class PlaylistRepository:
    ...
    def get_user_playlists(self, user_id: int, offset: int, limit: int) -> List[Playlist]:
        return self.db.query(Playlist).options(joinedload(Playlist.episodes)).filter(Playlist.user_id == user_id)\
    ...
    def get_playlist_by_id(self, playlist_id) -> Playlist | None:
        return self.db.query(Playlist).options(joinedload(Playlist.user))\
            .options(joinedload(Playlist.episodes).joinedload(PlaylistPodcast.podcast).joinedload(Podcast.channel))\
    ...
    def update_playlist(self, playlist: Playlist, title: Optional[str], description: Optional[str],
                        is_public: Optional[bool], cover_art_url: Optional[str] = None) -> Playlist:
    ...
    def get_playlist_episode(self, playlist_id: int, podcast_id: int) -> PlaylistPodcast | None:
        return self.db.query(PlaylistPodcast)\
            .filter(PlaylistPodcast.playlist_id == playlist_id, PlaylistPodcast.podcast_id == podcast_id).first()
    ...
    def get_collaborator(self, playlist_id: int, user_id: int) -> PlaylistCollaborator | None:
        return self.db.query(PlaylistCollaborator)\
            .filter(PlaylistCollaborator.playlist_id == playlist_id, PlaylistCollaborator.user_id == user_id).first()
