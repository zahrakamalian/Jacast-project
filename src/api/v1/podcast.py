...
def get_all_podcasts(service: Annotated[PodcastService, Depends(get_podcast_service)],
                     limit: int = 10,
                     page: int = 1):
...
def get_trending_podcasts(service: Annotated[PodcastService, Depends(get_podcast_service)],
                          limit: int = Query(10, ge=1, le=50)):
    return service.get_trending_podcasts(limit)


@router.get("/top-charts", response_model=List[PodcastDisplay])
def get_top_ranked_podcast(service: Annotated[PodcastService, Depends(get_podcast_service)],
                           limit: int = Query(10, ge=1, le=50)):
    return service.get_top_ranked_podcasts(limit)


@router.get("/new", response_model=List[PodcastDisplay])
def get_new_podcast(service: Annotated[PodcastService, Depends(get_podcast_service)],
                    limit: int = Query(10, ge=1, le=50)):
    return service.get_new_podcasts(limit)


@router.get("/{id}", response_model=PodcastDetail)
def get_podcast_details(service: Annotated[PodcastService, Depends(get_podcast_service)],
                        id: int):
    return service.get_podcast_details(id)


@router.post("/", response_model=PodcastDetail)
async def create_podcast(user: Annotated[User, Depends(get_current_user)],
...
async def update_podcast(user: Annotated[User, Depends(get_current_user)],
                         service: Annotated[PodcastService, Depends(get_podcast_service)],
                         id: int,
...
@router.delete("/{id}")
def delete_podcast(user: Annotated[User, Depends(get_current_user)],
                   service: Annotated[PodcastService, Depends(get_podcast_service)],
                   id: int):
...
def get_podcast_stats(service: Annotated[PodcastService, Depends(get_podcast_service)],
                      id: int):
    return service.get_podcast_stats(id)


@router.get("/{id}/subscribers", response_model=List[UserDisplay])
def podcast_subscribers(service: Annotated[PodcastService, Depends(get_podcast_service)],
                        id: int):
...
def add_podcast_review(user: Annotated[User, Depends(get_current_user)],
                       service: Annotated[PodcastService, Depends(get_podcast_service)],
...
def update_podcast_review(user: Annotated[User, Depends(get_current_user)],
                          service: Annotated[PodcastService, Depends(get_podcast_service)],
                          id: int,
                          review_id: int,
...
def delete_podcast_review(user: Annotated[User, Depends(get_current_user)],
                          service: Annotated[PodcastService, Depends(get_podcast_service)],
                          id: int,
                          review_id: int):
...
def report_podcast(user: Annotated[User, Depends(get_current_user)],
                   service: Annotated[PodcastService, Depends(get_podcast_service)],
                   id: int,
                   data: ReportCreate):
...
def create_share_link(service: Annotated[PodcastService, Depends(get_podcast_service)],
                      id: int):
    return service.create_share_link(id)


@router.get("/s/{token}")
def redirect_share_link(service: Annotated[PodcastService, Depends(get_podcast_service)],
                        token: str):
